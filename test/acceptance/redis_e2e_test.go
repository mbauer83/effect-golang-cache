...
var (
	...
	_ rate.Limiter = (*redis.Limiter)(nil)
)
...
// startMiniredis is a Redis this test owns, and the two things a program keeps in one.
func startMiniredis(t *testing.T) (*miniredis.Miniredis, *redis.Store, *redis.Limiter) {
	t.Helper()
	server := miniredis.RunT(t)
	server.SetTime(noon)
	client := goredis.NewClient(&goredis.Options{Addr: server.Addr()})
	t.Cleanup(func() { _ = client.Close() })
	return server, redis.NewStore(client), redis.NewLimiter(client)
}
...
func readBack(t *testing.T, store *redis.Store, key string) cache.Lookup {
	t.Helper()
	lookup, err := store.Get(context.Background(), key)
	if err != nil {
		t.Fatal(err)
	}
	return lookup
...
func putEntry(t *testing.T, store *redis.Store, entry cache.Entry) {
	t.Helper()
...
func TestAValueIsKeptUntilItStopsBeingWorthKeeping(t *testing.T) {
	server, store, _ := startMiniredis(t)
	...
	if lookup := readBack(t, store, "film:603"); !lookup.Found || string(lookup.Entity) != `{"said":"so"}` {
...
func TestAMissIsAnAnswerAndNotAFailure(t *testing.T) {
	...
	_, store, _ := startMiniredis(t)

	if lookup := readBack(t, store, "film:nobody-asked"); lookup.Found || len(lookup.Entity) != 0 {
...
func TestEverythingAboutOneSubjectIsForgottenAtOnce(t *testing.T) {
	...
	for _, entry := range entries[:2] {
		if gone := readBack(t, store, entry.Key); gone.Found {
			t.Fatalf("expected %q to have been forgotten", entry.Key)
		}
	}
	if still := readBack(t, store, entries[2].Key); !still.Found {
...
func TestAFilingWithNoLifetimeIsRefusedRatherThanKeptForever(t *testing.T) {
	...
	if lookup := readBack(t, store, "film:603"); lookup.Found {
...
func timeOneTurn(t *testing.T, limiter *redis.Limiter, allowance rate.Allowance) time.Duration {
	t.Helper()
...
func TestASpentAllowanceComesBackOneTurnAtATime(t *testing.T) {
	...
	server, _, limiter := startMiniredis(t)
	for range 4 {
		_ = timeOneTurn(t, limiter, threePerSecond())
	}
